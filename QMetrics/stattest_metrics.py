...
def nan_block(dfs, sample_type, output_dir):
    ...
    for df, label in list(zip(dfs, sample_type.split(','))):
        ...
        g = sns.histplot(data = df, x = '% NaN', kde = True, bins = len(df.columns) - 1)
...
def main():
    ...
    pars.add_argument('--fold-change', type = int, default = 2, help = 'fold_change threshold for static method')
    pars.add_argument('--alpha', default = 0.01, type = float, help = 'fdr_BH threshold for static method')
